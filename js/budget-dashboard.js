(function () {
  ...
  function renderLocalCurrency(rows) {
    const panel = document.getElementById("local-currency-summary");
    const localRows = rows.filter((row) => row.local_currency && Calc.hasValue(row.q2rf_budget_local));
    panel.hidden = localRows.length !== 1;
    if (localRows.length !== 1) return;
    const row = localRows[0];
    document.getElementById("local-currency-title").textContent = `${row.market.name} budget in source currency`;
    document.getElementById("local-currency-unit").textContent = row.local_currency;
    document.getElementById("local-budget-total").textContent = formatLocalCurrency(row.q2rf_budget_local, row.local_currency);
    document.getElementById("local-budget-studio").textContent = formatLocalCurrency(row.studio_budget_local, row.local_currency);
    document.getElementById("local-budget-kol").textContent = formatLocalCurrency(row.kol_budget_local, row.local_currency);
    document.getElementById("local-currency-note").textContent = Calc.hasValue(row.fx_rate_to_usd)
      ? `Original source values retained. Converted at 1 ${row.local_currency} = ${row.fx_rate_to_usd} USD on ${window.DashboardApp.formatDate(row.fx_rate_date)}; source: ${row.fx_rate_source || "documented exchange rate"}.`
      : `Confirmed source values. They remain outside consolidated USD000 totals until the official ${row.local_currency} to USD planning rate is supplied.`;
  }
  ...
  function render() {
    ...
    renderAlerts(data, rows); renderLocalCurrency(rows); renderKpis(rows, summary); renderCoverage(rows, summary); renderCharts(rows); renderTable(data, filters, rows);
  }
